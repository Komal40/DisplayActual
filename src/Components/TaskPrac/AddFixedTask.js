...
function AddFixedTask() {
  ...
  const updateTaskAssign = async () => {
    ...
    const taskArray = generateAllStations()
      ...
      .filter((station) => {
        const partSelected =
          selectedParts[station] !== undefined ||
          globalInputValue[selectedLine]?.part;
        const processSelected =
          selectedProcesses[station] !== undefined ||
          processName[selectedLine]?.[0]?.process_no;
        const quantityEntered =
          quantities[station] !== undefined ||
      ...
      .map((station, index) => ({
        ...
        quantity:
          quantities[station] || globalInputValue[selectedLine]?.inputValue,
  ...
  const handleGlobalCheckedBoxChange = (isChecked) => {
    if (activeBtn == "All") {
      ...
      const newCheckedStations =
        allStations &&
        (allStations[activeBtn] || []).reduce((acc, station) => {
          acc[station] = isChecked;
          return acc;
        }, {...checkedStations});
      setCheckedStations(newCheckedStations);
  ...
  const handleGlobalInputValueChange = (val) => {
    if (activeBtn == "All") {
      const newStations =
        allStations &&
        Object.values(allStations)
      ...
      setGlobalInput(newStations);
    } else {
      const newStations = (allStations[activeBtn] || []).reduce(
        (acc, station) => {
          acc[station] = val;
          return acc;
        }
            ...
            <div className="taskfixed_table_header">
              ...
              <div>
                ...
                <input
                  ...
                  // value={
                  //   selectedLine === 0 ?
                  //     (generateAllStations().every(station => globalInput[station]) ? globalInput[generateAllStations()[0]] : "") :
                  //     (stationData.stations && (stationData.stations[selectedLine] || []).every(station => globalInput[station]) ? globalInput[stationData.stations[selectedLine][0]] : "")
                  // }
                  // onChange={(e)=>handleGlobalInputValueChange(e.target.value)}
                />
              </div>
  ...
  {process.process_no}
              ...
              <div>
                <input
                  ...
                  checked={
                    activeBtn === "All"
                      ? generateAllStations().every(
                          (station) => checkedStations[station]
                        )
                      : allStations &&
                        (allStations[activeBtn] || []).every(
                          (station) => checkedStations[station]
                        )
                  }
                  onChange={(e) =>
                    handleGlobalCheckedBoxChange(e.target.checked)
                  }
                />
              </div>
            </div>
